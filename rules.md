...
必须在标题和内容中自然覆盖以下关键词：
`ChatGPT`, `ChatGPT官网`, `Chat GPT官网`, `chatgpt官网地址`, `chat gpt`, `gpt官网`, `openai官网`, `chatgpt中文版`, `GPT-5`, `GPT-4o`, `国内使用`
...
- 关键词尽量靠前放置
- 提供英文 URL slug：`product-keyword-guide`（包含英文关键词或拼音）
...
文章末尾提供路由配置代码块，根据生成的 `path`（文件名）和 `title` 生成 JSON 对象：
